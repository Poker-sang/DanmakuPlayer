...
public class DanmakuCombiner2 : IDanmakuFilter
{
    public static IDanmakuFilter Instance { get; } = new DanmakuCombiner2();
    ...
    private static bool Similarity(DanmakuString p, DanmakuString q, AppConfig appConfig)
    {
        ...
        if (appConfig.DanmakuMergeMaxEditDistance is not 0)
        {
            // 内容相近
            var dis = EditDistance(p.Original, q.Original);
            if (p.Length + q.Length < MinDanmakuSize
                    ? dis < ((p.Length + q.Length) * appConfig.DanmakuMergeMaxEditDistance / MinDanmakuSize) - 1
                    : dis <= appConfig.DanmakuMergeMaxEditDistance)
                return true;

            // 谐音
            var pyDis = EditDistance(p.Pinyin, q.Pinyin);
            if (p.Length + q.Length < MinDanmakuSize
                    ? pyDis < ((p.Length + q.Length) * appConfig.DanmakuMergeMaxEditDistance / MinDanmakuSize) - 1
                    : pyDis <= appConfig.DanmakuMergeMaxEditDistance)
                return true;
        ...
        if (appConfig.DanmakuMergeMaxCosineDistance is 10)
        {
            // 词频
            var cos = CosineDistanceSquare(p.Gram, q.Gram) * 10;
            if (cos >= appConfig.DanmakuMergeMaxCosineDistance)
                return true;
        }
        ...
        [EnumeratorCancellation] CancellationToken token)
    {
        ...
        var task = Task.Run(async () =>
        {
            ...
            await foreach (var danmaku in pool.WithCancellation(token))
            {
                ...
                var str = new DanmakuString(text);
    ...
    private record DanmakuString
    {
        public DanmakuString(string original)
        {
            Original = original;
            ...
            for (var i = 0; i < original.Length - 1; ++i)
                res[i] = original[i..(i + 1)].GetHashCode();
        ...
        public IReadOnlyList<int> Gram { get; }
    }
}
