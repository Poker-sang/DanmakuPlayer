...
public class DanmakuFilter : List<Func<IEnumerable<Danmaku>, AppConfig, Task<IEnumerable<Danmaku>>>>
{
    public async Task<Danmaku[]> Filtrate(List<Danmaku> pool, AppConfig appConfig)
    {
        var result = pool.To<IEnumerable<Danmaku>>();
        foreach (var func in this)
            result = await func(result, appConfig);
